...
import { ChevronLeft, ShoppingBag, Heart, Shield, RefreshCw, Package, Check } from 'lucide-react';
...
export const ProductDetail: React.FC<ProductDetailProps> = ({ 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-muted-foreground hover:text-teal transition-colors mb-8 cursor-pointer"
      >
        <ChevronLeft size={20} />
        Back to collection
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div className="flex flex-col">
          <div className="mb-8">
            <p className="text-coral font-bold text-xs tracking-[0.3em] uppercase mb-3">{product.category}</p>
            <h1 className="text-5xl font-bold text-teal mb-4 leading-tight">{product.name}</h1>
            <p className="text-4xl font-bold text-coral">{product.price} EGP</p>
          </div>

          <div className="mb-8">
            <div className="flex justify-between items-center mb-4">
              <span className="font-bold text-teal uppercase tracking-widest text-xs">Choose Color</span>
              ...
              {product.colors.map((color) => (
                <button
                  ...
                  className={`w-12 h-12 rounded-full border-2 transition-all flex items-center justify-center cursor-pointer ${
                    selectedColor.name === color.name ? 'border-coral p-1' : 'border-white bg-white shadow-sm'
                  }`}
                >
                  <div 
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-4">
              <span className="font-bold text-teal uppercase tracking-widest text-xs">Select Size</span>
              <button 
            ...
            <div className="flex flex-wrap gap-3">
              {product.sizes.map((size) => (
                <button
                  ...
                  className={`min-w-16 h-16 px-4 rounded-2xl border-2 font-bold transition-all cursor-pointer ${
                    selectedSize === size
          ...
          <div className="space-y-4 mb-12">
            <div className="flex gap-4">
              <button
                ...
                className={`flex-1 py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 shadow-xl transition-all cursor-pointer ${
                  addedStatus 
              ...
              <button 
                ...
                className={`p-5 rounded-2xl transition-all border-2 cursor-pointer ${
                  isFavorite 
            ...
            <p className="text-center text-[10px] text-teal/40 uppercase tracking-[0.2em] font-bold">
              Pay Cash on Delivery after inspection
            </p>
          </div>

          <div className="border-t border-teal/10 pt-8">
            <div className="flex gap-8 border-b border-teal/10 mb-6">
              {tabs.map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`pb-4 text-xs font-bold uppercase tracking-widest transition-all relative cursor-pointer ${
                    activeTab === tab ? 'text-teal' : 'text-teal/30 hover:text-teal/60'
                  }`}
                >
                  {tab}
                  {activeTab === tab && (
            ...
            <div className="min-h-[150px]">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="text-teal/70 leading-relaxed font-medium"
                >
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="grid grid-cols-2 gap-4">
                        {product.features.map((f, idx) => (
                          <li key={idx} className="flex items-center gap-3 text-sm bg-white/40 p-3 rounded-xl border border-teal/5">
                  ...
                  {activeTab === 'Materials & Care' && (
                    <div className="space-y-6">
                      <div className="bg-white/40 p-4 rounded-2xl border border-teal/5">
                        <p className="text-xs uppercase tracking-widest text-teal/40 mb-2">Composition</p>
                        <p className="font-bold text-teal">{product.materials}</p>
                      </div>
                      <p className="text-sm">Machine wash cold with like colors. Do not bleach. Tumble dry low. Do not iron.</p>
                    </div>
                  )}
                  {activeTab === 'Shipping & Privacy' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="flex gap-4 items-center bg-white/40 p-4 rounded-2xl border border-teal/5">
                        <div className="w-10 h-10 rounded-xl bg-teal/10 flex items-center justify-center flex-shrink-0">
                          <Package size={20} className="text-teal" />
                        </div>
                        <div>
                          <p className="font-bold text-sm text-teal">Discreet</p>
                          <p className="text-[10px] text-teal/50 uppercase tracking-tighter">Plain unmarked boxes</p>
                        </div>
                      </div>
                      <div className="flex gap-4 items-center bg-white/40 p-4 rounded-2xl border border-teal/5">
                        <div className="w-10 h-10 rounded-xl bg-teal/10 flex items-center justify-center flex-shrink-0">
                        ...
                        <div>
                          <p className="font-bold text-sm text-teal">Inspection</p>
                          <p className="text-[10px] text-teal/50 uppercase tracking-tighter">Pay after inspection</p>
                        </div>
                      </div>
                    </div>
                  )}
